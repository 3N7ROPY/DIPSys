...
BEGIN_MESSAGE_MAP(CColorGradWnd, CWnd)
	...
	ON_WM_MOUSEMOVE()
	//按钮
	ON_CBN_SELCHANGE(2005, &CColorGradWnd::OnChannelSelected)
	ON_BN_CLICKED(2001, &CColorGradWnd::OnClickOK)
	ON_BN_CLICKED(2002, &CColorGradWnd::OnClickCancel)
	//编辑控件失去输入焦点
	ON_EN_KILLFOCUS(2006, &CColorGradWnd::OnEditLeftTop)
	ON_EN_KILLFOCUS(2007, &CColorGradWnd::OnEditTop)
	ON_EN_KILLFOCUS(2008, &CColorGradWnd::OnEditRightTop)
	ON_EN_KILLFOCUS(2009, &CColorGradWnd::OnEditLeftBottom)
	ON_EN_KILLFOCUS(2010, &CColorGradWnd::OnEditRightBottom)
END_MESSAGE_MAP()
...
void CColorGradWnd::OnSize(UINT nType, int cx, int cy)
{
	...
	//RGB组合框
	if (m_pComboBoxRGB->GetSafeHwnd()) m_pComboBoxRGB->MoveWindow(50, 15, 90, 35);
...
int CColorGradWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_pBnOK->Create(L"确定", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(m_pClientRect->right-50, 20, m_pClientRect->right-10, 40), this, 2001);
	m_pBnOK->SetFont(m_pWndFont, TRUE);

	//取消
	m_pBnCancel = new CMFCButton;
	m_pBnCancel->Create(L"取消", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(m_pClientRect->right-50, 60, m_pClientRect->right-10, 80), this, 2002);
	m_pBnCancel->SetFont(m_pWndFont, TRUE);

	//上面的GroupBox
	m_pBnGroupBox = new CButton;
	m_pBnGroupBox->Create(L"通道:", WS_CHILD | WS_VISIBLE | BS_GROUPBOX, CRect(10, 20, m_pClientRect->right-60, m_pClientRect->bottom-10), this, 2003);
	m_pBnGroupBox->SetFont(m_pWndFont, TRUE);

	//输入色阶
	m_pStaticInput = new CStatic;
	m_pStaticInput->Create(L"输入色阶:", WS_CHILD | WS_VISIBLE, CRect(20, 40, 80, 60), this, 2004);
	m_pStaticInput->SetFont(m_pWndFont, TRUE);

	//RGB组合框
	m_pComboBoxRGB = new CComboBox;
	m_pComboBoxRGB->Create(WS_CHILD | WS_VISIBLE | CBS_AUTOHSCROLL | CBS_DROPDOWNLIST | WS_EX_TOPMOST, CRect(50, 15, 90, 35), this, 2005);
	...
	m_pEditLeftTop->Create(WS_CHILD | WS_VISIBLE | ES_NUMBER, CRect(m_pPaintRect->left, m_pPaintRect->bottom+20, m_pPaintRect->left+40, m_pPaintRect->bottom+40), this, 2006);
	m_pEditLeftTop->SetFont(m_pWndFont, TRUE);

	//正上方的编辑框
	m_pEditTop = new CEdit;
	m_pEditTop->Create(WS_CHILD | WS_VISIBLE, CRect(int(m_pPaintRect->left+m_pPaintRect->Width()/2.0-20), m_pPaintRect->bottom+20, int(m_pPaintRect->left+m_pPaintRect->Width()/2.0+20), m_pPaintRect->bottom+40), this, 2007);
	m_pEditTop->SetFont(m_pWndFont, TRUE);

	//右上角的编辑框
	m_pEditRightTop = new CEdit;
	m_pEditRightTop->Create(WS_CHILD | WS_VISIBLE | ES_NUMBER, CRect(m_pPaintRect->right-40, m_pPaintRect->bottom+20, m_pPaintRect->right, m_pPaintRect->bottom+40), this, 2008);
	m_pEditRightTop->SetFont(m_pWndFont, TRUE);

	//左下角的编辑框
	m_pEditLeftBottom = new CEdit;
	m_pEditLeftBottom->Create(WS_CHILD | WS_VISIBLE | ES_NUMBER, CRect(m_pPaintRect->left, m_pClientRect->bottom-40, m_pPaintRect->left+40, m_pClientRect->bottom-20), this, 2009);
	m_pEditLeftBottom->SetFont(m_pWndFont, TRUE);

	//右下角的编辑框
	m_pEditRightBottom = new CEdit;
	m_pEditRightBottom->Create(WS_CHILD | WS_VISIBLE | ES_NUMBER, CRect(m_pPaintRect->right-40, m_pClientRect->bottom-40, m_pPaintRect->right, m_pClientRect->bottom-20), this, 2010);
	m_pEditRightBottom->SetFont(m_pWndFont, TRUE);

	//输出色阶
	m_pStaticOutput = new CStatic;
	m_pStaticOutput->Create(L"输出色阶:", WS_CHILD | WS_VISIBLE, CRect(m_pPaintRect->left+10, m_pPaintRect->bottom+60, m_pPaintRect->left+70, m_pPaintRect->bottom+80), this, 2013);
...
//获取主程序指针，通过指针能访问其公有成员
CDIPSysDlg* CColorGradWnd::GetDocument()
{
...
void CColorGradWnd::ShowEditValue()
{
	...
	for (int i = 5; i<10; i++)//CEdit在CWnd中的排序
	{
		...
		m_WndVector[i]->SetWindowTextW(text);
	}
}
...
void CColorGradWnd::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, 2006, m_dValue[0]);
	DDX_Text(pDX, 2007, m_dMedian);
	DDX_Text(pDX, 2008, m_dValue[2]);
	DDX_Text(pDX, 2009, m_dValue[3]);
	DDX_Text(pDX, 2010, m_dValue[4]);
	//看数据是否合法
	int flag = 1;
