...
BEGIN_MESSAGE_MAP(CSpaceFilterWnd, CWnd)
	...
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_BN_CLICKED(2201, &CSpaceFilterWnd::OnCmdOK)
	ON_BN_CLICKED(2202, &CSpaceFilterWnd::OnCmdCancel)
	ON_BN_CLICKED(2203, &CSpaceFilterWnd::OnCmdReset)
	ON_BN_CLICKED(2204, &CSpaceFilterWnd::OnCmdMedianFilter)
	ON_BN_CLICKED(2205, &CSpaceFilterWnd::OnCmdAdaptiveFilter)
	ON_BN_CLICKED(2206, &CSpaceFilterWnd::OnCmdAddPepperSaltNoise)
	ON_BN_CLICKED(2207, &CSpaceFilterWnd::OnCmdAddGaussianNoise)
	ON_BN_CLICKED(2297, &CSpaceFilterWnd::OnCmdApply)
	ON_CBN_SELCHANGE(2298, &CSpaceFilterWnd::OnCmdChooseOperation)
	ON_CBN_SELCHANGE(2299, &CSpaceFilterWnd::OnCmdChangeFilterSize)
END_MESSAGE_MAP()
...
int CSpaceFilterWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_pBnOK->Create(L"确  定", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(), this, 2201);
	m_pBnOK->SetFont(m_pAppFont);

	//取消
	m_pBnCancel = new CMFCButton;
	m_pBnCancel->Create(L"取  消", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(), this, 2202);
	m_pBnCancel->SetFont(m_pAppFont);

	//重置
	m_pBnReset = new CMFCButton;
	m_pBnReset->Create(L"重  置", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(), this, 2203);
	m_pBnReset->SetFont(m_pAppFont);

	//中值滤波
	m_pBnMedianFilter = new CMFCButton;
	m_pBnMedianFilter->Create(L"中值滤波", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(), this, 2204);
	m_pBnMedianFilter->SetFont(m_pAppFont);

	//自适应中值滤波
	m_pBnAdaptiveFilter = new CMFCButton;
	m_pBnAdaptiveFilter->Create(L"自适应", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(), this, 2205);
	m_pBnAdaptiveFilter->SetFont(m_pAppFont);

	//添加椒盐噪声
	m_pBnAddPepperSaltNoise= new CMFCButton;
	m_pBnAddPepperSaltNoise->Create(L"椒盐噪声", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(), this, 2206);
	m_pBnAddPepperSaltNoise->SetFont(m_pAppFont);

	//添加高斯噪声
	m_pBnAddGaussianNoise = new CMFCButton;
	m_pBnAddGaussianNoise->Create(L"高斯噪声", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(), this, 2207);
	m_pBnAddGaussianNoise->SetFont(m_pAppFont);

	//应用操作
	m_pBnApply = new CMFCButton;
	m_pBnApply->Create(L"应  用", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(), this, 2297);
	m_pBnApply->SetFont(m_pAppFont);

	//选择操作类型
	m_pComboOperation = new CComboBox;
	m_pComboOperation->Create(WS_CHILD | WS_VISIBLE | CBS_AUTOHSCROLL | CBS_DROPDOWNLIST | WS_EX_TOPMOST, CRect(), this, 2298);
	m_pComboOperation->InsertString(0, L"Operate");
	...
	m_pComboFilterSize->Create(WS_CHILD | WS_VISIBLE | CBS_AUTOHSCROLL | CBS_DROPDOWNLIST | WS_EX_TOPMOST, CRect(), this, 2299);
	...
	for (int i = 0; i<MAX_FILTER_SIZE; i++)
	{
		for (int j = 0; j<MAX_FILTER_SIZE; j++)
		{
			...
			m_pElements[j + i*MAX_FILTER_SIZE]->Create(WS_CHILD | WS_VISIBLE, CRect(150+(j+1)*SpaceX, (i+1)*SpaceY, 150+(j+1)*SpaceX+40, (i+1)*SpaceY+10), this, 2210+i);
	...
	for (int i = R-r, s = 0; i<=R+r; i++, s++)
	{
		...
		{
			m_pElements[j + i*MAX_FILTER_SIZE]->SetWindowTextW(L"0");
			m_pElements[j + i*MAX_FILTER_SIZE]->ShowWindow(SW_SHOW);
		}
	}

	//获取主程序指针
	CDIPSysDlg* pDoc = (CDIPSysDlg*)GetParent()->GetParent();
...
void CSpaceFilterWnd::OnSize(UINT nType, int cx, int cy)
{
	...
	Reshape(m_pComboOperation, Space, 4, 2);
	Reshape(m_pComboFilterSize, Space, 5, 2);
...
afx_msg void CSpaceFilterWnd::OnCmdReset()
{
	...
	if (m_pImage != NULL)
	{
		...
		((CDIPSysDlg *)GetParent()->GetParent())->Invalidate(TRUE);
	}
}
...
afx_msg void CSpaceFilterWnd::OnCmdApply()
{
	...
	for (int i = R-r, s = 0; i<=R+r; i++, s++)
	{
		for (int j = R-r, t = 0; j<=R+r; j++, t++)
		{
			m_pElements[j + i*MAX_FILTER_SIZE]->GetWindowTextW(text);
			m_pMyFilter->Visit(t, s) = _wtof(text.GetBuffer());
		}
	}
...
afx_msg void CSpaceFilterWnd::OnCmdChooseOperation()
{
	...
	for (int i = R-r, s = 0, k = 0; i<=R+r; i++, s++)
	{
		for (int j = R-r, t = 0; j<=R+r; j++, t++)
		{
			...
			m_pElements[j + i*MAX_FILTER_SIZE]->SetWindowTextW(text);
			m_pElements[j + i*MAX_FILTER_SIZE]->ShowWindow(SW_SHOW);
		}
	}
}
...
afx_msg void CSpaceFilterWnd::OnCmdChangeFilterSize()
{
	...
	for (int i = 0; i<MAX_FILTER_SIZE*MAX_FILTER_SIZE; i++)
	{
		...
		m_pElements[i]->Invalidate(FALSE);
	}
	// 设置显示
	int R = MAX_FILTER_SIZE/2;
