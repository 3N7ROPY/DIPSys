...
BEGIN_MESSAGE_MAP(CHistogramWnd, CWnd)
	...
	ON_WM_PAINT()
	ON_WM_SIZE()
	ON_BN_CLICKED(2101, &CHistogramWnd::OnCmdOK)
	ON_BN_CLICKED(2102, &CHistogramWnd::OnCmdCancel)
	ON_BN_CLICKED(2103, &CHistogramWnd::OnCmdHe)
	ON_BN_CLICKED(2104, &CHistogramWnd::OnCmdLhe)
	ON_BN_CLICKED(2105, &CHistogramWnd::OnCmdAhe)
	ON_BN_CLICKED(2106, &CHistogramWnd::OnCmdClahe)
	ON_BN_CLICKED(2107, &CHistogramWnd::OnCmdReset)
END_MESSAGE_MAP()


//获取主程序指针，通过指针能访问其公有成员
CDIPSysDlg* CHistogramWnd::GetDocument()
{
...
int CHistogramWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_pBnOK->Create(L"确定", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(20, Space-10, 80, Space+10), this, 2101);

	//取消
	m_pBnCancel = new CMFCButton;
	m_pBnCancel->Create(L"取消", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(20, 2*Space-10, 80, 2*Space+10), this, 2102);

	//HE
	m_pBnHE = new CMFCButton;
	m_pBnHE->Create(L"HE", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(20, 3*Space-10, 80, 3*Space+10), this, 2103);

	//LHE
	m_pBnLHE = new CMFCButton;
	m_pBnLHE->Create(L"LHE", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(20, 4*Space-10, 80, 4*Space+10), this, 2104);

	//AHE
	m_pBnAHE = new CMFCButton;
	m_pBnAHE->Create(L"AHE", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(20, 5*Space-10, 80, 5*Space+10), this, 2105);

	//CLAHE
	m_pBnCLAHE = new CMFCButton;
	m_pBnCLAHE->Create(L"CLAHE", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(20, 6*Space-10, 80, 6*Space+10), this, 2106);

	//CLAHE
	m_pBnReset = new CMFCButton;
	m_pBnReset->Create(L"重置", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(20, 7*Space-10, 80, 7*Space+10), this, 2107);
...
void Write2File(CString FilePath, TYPE* dataSrc, int Width, int Height)//写入文件
{
	...
	{
		for (int i = 0; i<Width; i++) Output<<(double)*(dataSrc + i + j*Width)<<", ";
		Output<<"\n";
	}
	Output.close();
}
